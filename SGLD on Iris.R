...
ptmbegin = proc.time()
output = SGLD(init_beta(),x_training,y_training,breaks,batchsize)
proc.time() - ptmbegin
final_beta = output[breaks,]

for (i in 2:divisions){
  ptm = proc.time()
  new_output = SGLD(final_beta,x_training,y_training,breaks,batchsize, t = breaks*(i-1))
  print(proc.time() - ptm)
...
output_training = outputs(output[burn:iter,],x_training,y_training)
correct_training = accuracy(output_training, y_training)

output_test =  outputs(output[burn:iter,],x_test,y_test)
